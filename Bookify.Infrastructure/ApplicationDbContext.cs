...
//	private readonly IPublisher _publisher;
	...
	// Constructor accepts the DBContextOption class, and it is passing it to the BaseClass constructor, accepting this
	// type.
...
//	}
														   ...
														   = default(CancellationToken) )
	{
		...
		{
			// Transactional Outbox Pattern
			// We are calling this method right before we call saveChangesAsync which will make sure to 
			// load the domain events into the outbox messages and add them to the change tracker, and once
			// we call saveChanges we will be persisting everything to the database in a single transaction which 
			// gives us automatic guarantees because we are using SQL database. So either all of the outbox
			// messages are persisted together as part of our transaction, or nothing is persisted at all, and
			// the end result is that the system remains in a consistent state. 
			...
			// What we are doing here is potentially problematic. We are saving changes to the database, and okay this
			// is atomic,
			// and this is either going to succeed or fail, but when we are publishing a set of domain events which is
			// another
			...
			// However, the original call to the database was already successfully completed. So in the case of a domain
			// event handler
			// failure, it would appear as though the whole transaction failed, but that wouldn't be the case. 
			// This is a major concern with this approach, and it's something that we need to be aware of if we want to
			// be running this
			// in production. There are also implementations that go ahead and publish the domain events before calling
			// SaveChangesAsync The reasoning
			// is then the handling of the domain events becomes part of the overall transaction by EF Core. While this
			// is inherently true, the 
			// whole concept of an event is something that is a fact. An event is something that has already happened in
			// our system, and publishing
			// the domain events before actually persiting the original changes that triggered these events makes
			// absolutely no sense. 
			// Which is why we go ahead and publish our domain events only after persisting these changes in the
			// database. 
...
//			await PublishDomainEventsAsync();
		...
		catch ( DbUpdateConcurrencyException ex )
		{
			...
			// So now our database context, which is unit of work, is going to be handling the
	...
	// Transactional Outbox Pattern
	// We changed the method signature to non async because we are only adding stuff to the change tracker
...
//			ToList();
		...
		var outboxMessages = ChangeTracker
		   ...
		   .
			...
			// Converting outbox messages and store them in the database
			// Adding projection where we are going to take the domain event
			// instance and create an outbox message. 
			Select (
...
//		}
		...
		// Adding messages to the change tracker, passing a list of outbox messages to addrange method. 
		AddRange (
