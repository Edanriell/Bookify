...
internal sealed class CustomClaimsTransformation : IClaimsTransformation
{
	...
	{
		bool hasRoleClaim = principal.HasClaim(c => c.Type == ClaimTypes.Role);
		bool hasSubClaim = principal.HasClaim(c => c.Type == JwtRegisteredClaimNames.Sub);

		bool isUnauthenticated = principal.Identity is not { IsAuthenticated: true };
		bool hasBothClaims = hasRoleClaim && hasSubClaim;

		if (isUnauthenticated || hasBothClaims)
		{
			return principal;
		}
